...
> Answer:  The benefits of using client-side routing is it offers control over the site URL and content without necessarily having to wait for a server to respond with content.  We can maintain State(memory) within our applications and use that memory to tell the Browser what to display when a resource is requested. Therefore our information is ready for us at any moment, the page doesn't have to refresh/reload(waiting for a response from a sever) .  It is faster because less data is being processed.  It also has smooth transitions and animations between views which are easier to implement
